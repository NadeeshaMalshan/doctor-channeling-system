...
/**
 * Same jsPDF output as src/utils/generateReceiptPDF.js (for email attachment in Node).
 */
const fs = require('fs');
const path = require('path');
const { jsPDF } = require('jspdf');
const { getReceiptDetailRows, RECEIPT_CONDITIONS } = require('./receiptModel');
...
function loadLogoDataUrlSync() {
    const logoPath = path.join(__dirname, '..', '..', 'src', 'images', 'LogoHospital.png');
...
/**
 * @param {object} data same shape as generateReceiptPDF (paymentID, appointmentId, …)
 * @returns {Buffer}
 */
function buildReceiptPdfBufferFromJsPDF(data) {
    ...
    const logoData = loadLogoDataUrlSync();
    if (logoData) {
        try {
    ...
    doc.text('Thank you for choosing NCC eCare. We wish you good health.', W / 2, y + 9, { align: 'center' });
    ...
    totalTextH += conditions.length * 2;
    ...
    doc.text(`Generated: ${now.toLocaleString('en-US')}`, W - M, y + 5, { align: 'right' });

    const arr = doc.output('arraybuffer');
    return Buffer.from(arr);
}

module.exports = { buildReceiptPdfBufferFromJsPDF };
